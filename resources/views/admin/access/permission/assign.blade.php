							...
							@foreach($permissions as $title => $items)
								<div class="card">
									<div class="card-header" id="headingOne{{ str_slug($title) }}">
										<h2 class="mb-0">
											<button class="btn btn-white" type="button" data-toggle="collapse" data-target="#collapseOne{{ str_slug($title) }}" aria-expanded="true" aria-controls="collapseOne{{ str_slug($title) }}">
												+ {{ $title }}
											</button>
										</h2>
									</div>
										...
										<div class="card-body">
											<ul>
												@foreach($items as $item)
													<li title="{{ $item['slug'] }}">
														<input type="checkbox" name="permissions[]" value="{{ $item['slug'] }}" {{ in_array($item['slug'], $role->permissions->toArray()) ? 'checked' : '' }}>
