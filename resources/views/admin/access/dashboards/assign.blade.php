    ...
    <div class="col col-lg-6">
        <div class="card">
			<form action="" method="post">
				...
				@method('PUT')
				<div class="card-body">
					<h4 class="card-title">{{ $role->name }} can see</h4>
										...
										<div class="card-body">
											<ul>
												@foreach($items as $item)
													<li title="{{ $item['slug'] }}">
														<input type="checkbox" name="dashboards[]" value="{{ $item['slug'] }}" {{ isset($role->dashboard_access) && in_array($item['slug'], $role->dashboard_access->toArray()) ? 'checked' : '' }}>
